-- 030_fix_search_members_bucket_cast.sql
--
-- Fixes error 42804 from migration 029: member_profile.bucket is actually a
-- Postgres enum type (icp_bucket), not text. The function's declared return
-- type (bucket text) didn't match the real column type, so Postgres refused
-- to run it at all. Fix: explicitly cast to text.

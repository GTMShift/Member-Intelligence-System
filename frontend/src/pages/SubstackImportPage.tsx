    ...
    <div className="flex min-h-screen flex-col">
      <header className="border-b border-slate-200 bg-white">
        <div className="mx-auto flex max-w-[90rem] items-center justify-between px-4 py-4 sm:px-6">
          <div>
            ...
            <p className="text-sm text-slate-500">Substack subscriber import</p>
          </div>
          <button
            type="button"
            onClick={() => navigate('/')}
      ...
      <main className="mx-auto w-full max-w-2xl flex-1 px-4 py-8 sm:px-6">
        <section className="rounded-xl border border-slate-200 bg-white p-6">
          ...
          <label
            htmlFor="csv-upload"
            className="flex cursor-pointer flex-col items-center justify-center gap-2 rounded-lg border-2 border-dashed border-slate-300 py-10 transition-colors hover:border-slate-400"
          >
          ...
          <button
            ...
            className="mt-4 w-full rounded-md bg-slate-900 py-2.5 text-sm font-medium text-white hover:bg-slate-700 disabled:opacity-50"
          >
          ...
          {result && (
            <div className="mt-4 space-y-1 rounded-lg border border-green-200 bg-green-50 px-4 py-3">
              <p className="text-sm font-medium text-green-800">Import complete</p>
              <p className="text-sm text-green-700">{result.total_rows} rows processed</p>
              <p className="text-sm text-green-700">{result.new} new subscribers</p>
              <p className="text-sm text-green-700">{result.reactivated} reactivated</p>
              <p className="text-sm text-green-700">{result.unsubscribed} marked unsubscribed</p>
              <p className="text-sm text-green-700">{result.matched_to_members} linked to existing members</p>
              <p className="text-sm text-green-700">{result.engagement_snapshots_created} engagement snapshots recorded</p>
            </div>
          )}
        </section>
